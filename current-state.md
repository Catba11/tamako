...
- Verification: `cargo build --workspace`, `cargo test --workspace`
  (1158 tests, 0 failures, on main at decision 113; the live-API
...
113. (2026-09-10, operator-ruled) Bounded-concurrent single-text
    ...
    two error policies) backs the adapter's `embed_texts` override
    (pre-screen + deep recall — FAIL-FAST: the first error fails the
    call and truncates the batch, so a failing endpoint receives at
    most one in-flight window, the decision-66 sequential loop's
    failure-path load profile — not a whole-batch dispatch into an
    outage) and the drain tick's embed phase (TRY-ALL: every row
    attempted even after an error); the drain keeps exact per-row
    error attribution through a phase split (contents read,
...
123. (2026-09-18) Member join/leave events persist and render into
    ...
    persisted (Runtime). Passivity is complete: tail_stats skips
    join/leave rows (membership churn never fires a digest; the batch
    still includes them), and count_inbound_after excludes them (an
    arrival never makes a pending reply stale and never forces a
...
8. **Concept alias ambiguity collapses to the deterministic id.** An
   ...
   fragment nodes).
9. **`ChatMember` updates are ignored.** RE-DEFERRED (M6). Correction
   (2026-09-18): the poller DOES subscribe `AllowedUpdate::ChatMember`
   (adapter.rs) — the updates arrive and are debug-logged and dropped;
   there is no consumer. (Join/leave SERVICE messages are a different,
   consumed channel since decision 123.) Membership is re-evaluated
   with `getChatMember` at every startup (decision 29). The soak
   runbook watches capability through the startup log lines.
