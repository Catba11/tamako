...
6. **执行偏差记录**（相对本方案）：(a) 参照模型 haiku→glm-5.3-flash /
   ...
   本文档中以 g0–g7 假名化；(e) UC-3 的候选组装为「向量 KNN + FTS +
   两跳」简化复刻，不含唤醒人入口（发送人/回复目标邻边）与别名入口 —
   即 decision 124 的 Wake 类来源 — 因此无法复现发送人洪流饥饿，也不
   承载 decision 124 的呈交语义，decision 124 的验收改由生产副本上的
   双场景回放承担（recall_replay 探针：候选池 deduped=2,319/2,334，
   呈交 38 条且含目标事实）；(f) UC-3 fixture 在 cap 处曾经不确定：
   HashMap 折叠顺序 + 非全序 valid_at 排序 + truncate(40)，同一二进
   制两次运行 39/89 窗口候选集不同（2026-09-18 实测）；已修复为生产
   的一出现顺序折叠 + 确定性截断，fixture 自此可重现。
