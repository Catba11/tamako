...
1. Candidate pairs come from the vector index: cosine similarity at or above `merge_candidate_threshold` (0.85, provisional — recalibration pending like all thresholds after the decision-81 model switch; refer to Section 7.4), kind-compatible pairs only (Person↔Person, Concept↔Concept). Pairs already linked by `known_as` are excluded — those are legitimate surface-form links, not duplicates.
...
6. The `related_pairs` side table (decision 83, migration v12): one row per `related` verdict — the unordered pair (`node_a_id < node_b_id`, UNIQUE), reason, confirmed_by, a `status` ('pending'/'promoted'/'dismissed'), and the timestamp. INSERT OR IGNORE, first-write-wins. NO read path touches it (not recall, not resolution, not status): it is write-only state awaiting a future promotion pass that shows the digest model a pending pair WITH full batch text and lets it ground a real relationship edge (digest's open vocabulary), flipping the row to 'promoted'. Operator dismissal flips to 'dismissed'. Accepted regression: cross-language synonyms (ruled `related`, never `same`, by the decision-74 prompt) lose their entity-resolution alias binding until that pass — no wrong bindings occur, which is the safe direction.
