...
3. Do a vector search on name and description embeddings in the sidecar index. The score is cosine similarity. The thresholds are PROVISIONAL, tuned against live operation (current-state.md decisions 66 and 73). Decision 81 (the embedding-model switch to `google/gemini-embedding-2`, 3072 dimensions, migration v11) RESETS their calibration status: the numeric values below stay as the starting point, but they were tuned against qwen3-embedding-8b's score distribution and must be recalibrated against Gemini's — the `vector_resolution_*` counters and the merge-tool candidate corpus are the instruments, and the first post-switch deploy window is the calibration run.
