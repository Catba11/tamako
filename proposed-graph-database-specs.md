...
3. Do a vector search on name and description embeddings in the sidecar index. The score is cosine similarity. The band discipline (decision 104, calibrated against the 2026-09-07 offline distribution evaluation of the live groups — Gemini's score distribution compresses upward, and no score separates Concept identity from topical relatedness: false Concept pairs reach 0.989 while true duplicates sit as low as 0.945):
   - If the top score is `vector_candidate_threshold` (0.88) or more, do ONE budget-capped LLM confirmation call on the digest purpose — for EVERY kind. Decision 104 extends the decision-79 Person rule to Concept: a wrong binding is social/structural damage the merge tool cannot cleanly undo, so nothing auto-binds. An Alias candidate confirms against its target. An accept binds; a reject falls through.
   - A per-batch budget (`resolution_confirm_budget`, default 5) caps the confirmation calls; a batch that exhausts the budget treats the remaining candidate entities as below-threshold. A wrong binding is worse than a missing fact; fragmentation is repairable by the merge tool.
...
5. Write the embeddings of the name and the description of each Person, Alias, and Concept to the sidecar vector index. NOT in the graph transaction (current-state.md decision 66): after the commit, enqueue each new or changed node into the `pending_embeddings` queue of `specs.md` Section 5.2, best-effort. A background worker drains the queue, calls the embeddings endpoint (default pair: OpenRouter + `google/gemini-embedding-2` at its native 3072 dimensions — decision 81; the dimension is pinned and the index is rebuilt by migration when it changes), and writes the vector index. A startup reconciliation pass diffs graph nodes against embedded content hashes and enqueues the missing or stale ones, and drops vector and queue rows whose node no longer exists. Backfill, steady-state repair, and merge-tombstone cleanup all ride this one mechanism.
...
1. Candidate pairs come from the vector index: cosine similarity at or above `merge_candidate_threshold` (0.90, calibrated by decision 104; refer to Section 7.4), kind-compatible pairs only (Person↔Person, Concept↔Concept). Pairs already linked by `known_as` are excluded — those are legitimate surface-form links, not duplicates.
2. Each candidate pair gets one LLM confirmation with a three-way verdict: `same` merges, `related` records the pair in the `related_pairs` side table and creates NO graph edge (decision 83 — the confirmation sees only an isolated pair's name/kind/description, too little context to ground a real relationship name; the old `also_known_as` mapping was a semantic error: alias edges BIND in entity resolution, so a merely-associated pair could merge by the back door), `different` skips. The tool is dry-run by default; `--apply` executes the plan. A manual `--merge` form takes an operator-chosen pair directly.
3. One merge (`merge_nodes(loser, survivor)`) executes in this order, serialized per group:
   ...
   - Hard-delete the loser (DETACH DELETE). Delete its vector-index row and its queue rows. Rewrite the loser's `related_pairs` rows to the survivor in the same apply (INSERT OR IGNORE dedups; self-pairs drop; decision 83(c)).
...
6. The `related_pairs` side table (decision 83, migration v12): one row per `related` verdict — the unordered pair (`node_a_id < node_b_id`, UNIQUE), reason, confirmed_by, a `status` ('pending'/'promoted'/'dismissed'), and the timestamp. INSERT OR IGNORE, first-write-wins. The decision-106 promotion pass reads the `pending` rows at digest assembly: a pair whose two endpoint names both appear in the batch text rides the extraction prompt (cap 10 per batch), and an edge the digest model grounds for it lands DIRECTLY on the pair's stored node ids (deterministic binding, never entity resolution) with `promoted_from_related_pair` in the properties; a successful commit flips the row to 'promoted'. The offline `--dismiss-related-pair` flips a pending row to 'dismissed'. No OTHER read path touches the table (not recall, not resolution, not status). Accepted regression: cross-language synonyms (ruled `related`, never `same`, by the decision-74 prompt) lose their entity-resolution alias binding until that pass — no wrong bindings occur, which is the safe direction.
