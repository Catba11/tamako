...
- The decision-59/64 parrot filter then applies to the extracted text. Residual fence tokens are hygiene, not speech: a tag-only `<{pet}>`/`</{pet}>` line drops, an inline pair unwraps to its content, an edge token strips; a mid-line single token survives untouched (quotation protection — members discuss AI glitch output). Decision 95: the tag-only test keys on the opener's first `>` ENDING the line (trimmed) — an attribute-carrying inline pair like `<{pet} at="..">come eat</{pet}>` unwraps instead of dropping (pre-fix it was misread as tag-only and dropped, which on the fence-less warmup path lost the whole output). There is deliberately NO strip region for the pet tag — its residuals are fence hygiene by design — while a legacy `<you>` region stays for stored pre-95 summaries and memories quoting the old shape.
