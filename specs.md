...
4. If the decision is to participate, generate a reply with the main model. The output contract of Section 9.8 applies. Send it through the adapter. Write the bot's own message to the raw log and append it to the context. Rule B1 applies.
...
- The ephemeral reply instruction of step 4 requires the whole reply in exactly one `<reply>...</reply>` element, nothing outside it (decision 93). The fence sentence lives in the reply instruction only: the shared context-format gloss also feeds the JSON-outputting gates, so it never carries the fence. The decision-85 examples (Section 5.3) render the same wrapper — the contract formalizes a shape the prompt already teaches (the live model fenced spontaneously before the contract existed).
- Fence extraction runs at the reply validation seam, before the parrot filter: the FIRST complete `<reply>...</reply>` pair yields the reply text (an attribute-carrying `<reply ...>` opener is accepted — the instruction names a message id, so an imitated attribute is an expected variant); everything outside the fence drops, with one WARN carrying the dropped byte count. The fence is the allowlist complement of the markup blocklist below: an UNKNOWN future reasoning marker outside the fence drops with no code change.
- The contract is FAIL-OPEN by design: an absent, unclosed, or malformed fence passes the whole text through with one WARN. A non-compliant model degrades to the pre-contract hygiene level instead of dropping every reply (the deployment runs several endpoints and models; strict fence-or-drop would turn a model swap into silence). The warmup path carries no fence sentence and keeps the pre-contract handling.
- The decision-59/64 parrot filter then applies to the extracted text. Residual fence tokens are hygiene, not speech: a tag-only `<reply>`/`</reply>` line drops, an inline pair unwraps to its content, an edge token strips; a mid-line single token survives untouched (quotation protection — members discuss AI glitch output).
- Independently of the fence, the endpoint layer strips reasoning markup from the extracted text of EVERY completion purpose (reply, gates, summary, extraction, caption) before purpose-specific handling (decision 93): balanced `<think>...</think>` regions strip; an orphan `</think>` drops everything up to and including it (the observed shape of a provider-side reasoning parser splitting at a literal `</think>` the reasoning itself mentioned); an unclosed `<think>` voids the remainder, so an all-reasoning response maps to the same extraction error as a text-less response and every caller's backoff and dead-letter discipline applies unchanged. One WARN per non-trivial strip. Providers that return reasoning on a separate response field were already safe: the client maps it to a content variant the text extraction never selects.
