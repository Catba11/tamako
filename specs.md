...
| `embedding_concurrency` | 1 | Bound on in-flight SINGLE-TEXT embedding POSTs (decision 113), PER CALL SITE — every vector pre-screen/deep-recall batch and every drain tick bounds itself, so concurrently active groups sum (not a process-wide cap). Concurrent single-text requests, never array input (the decision-81 ZDR route serves single-text only). Global only. 1 = the decision-66 sequential behavior. |
