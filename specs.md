...
- The contract is FAIL-OPEN by design: an absent, unclosed, or malformed fence passes the whole text through with one WARN. A non-compliant model degrades to the pre-contract hygiene level instead of dropping every reply (the deployment runs several endpoints and models; strict fence-or-drop would turn a model swap into silence). The warmup path carries no fence sentence and keeps the pre-contract handling. Decision 96 (C4): the seam WARNs of this section carry the purpose and the resolved model name; the fence-fallback WARN also names the expected opener (decision 95's `fence_open` field).
