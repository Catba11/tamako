...
use tamako_core::wake::{
    extract_reply_fence, filter_reply_parrot_lines, GateMessage, ReplyFence, ReplyFilterOutcome,
...
/// `<{pet}>...</{pet}>` element, extracted by `extract_reply_fence`
...
/// Decision 95 unifies the fence tag with the own-speech tag: both
/// derive from the persona name (`tamako_persona::pet_tag_for_name`,
/// threaded here as `pet_tag`), so every rendered own-history item is
/// one more demonstration of the fence shape.
///
...
/// the `<msg>` context shape, so the model is told never to
...
pub fn render_reply_instruction(target: &GateMessage, pet_tag: &str) -> String {
    ...
    format!(
        "{reference}\n\
         ...
         Wrap the whole reply in exactly one <{pet_tag}>...</{pet_tag}> element \
         and write nothing outside it. \
         Never write \"I remember:\" lines, <memory> blocks, <summary> blocks, \
         <msg> blocks, or a memory list: \
...
fn assemble_reply_messages(
    ...
    let base_instruction = render_reply_instruction(&request.target, pet_tag);
...
pub fn trimmed_reply_or_error(
    ...
    // Decision 95: the fence is the pet tag, derived from the persona
    // name upstream and threaded in (tamako_persona::pet_tag_for_name).
...
pub struct RigReplyGenerator {
    ...
    timezone: Option<ResolvedTimezone>,
    /// The decision-95 pet tag (the own-speech / fence element name),
    /// behind a shared lock so the decision-80 persona hot reload swaps
    /// it in place — a persona `name` change flips the tag on the next
    /// request with no reconstruction. Read at request-assembly time;
    /// never persisted. The default is the legacy fallback `"you"`
    /// (`tamako_persona::pet_tag_for_name` returns it for a name with
    /// no ASCII keepers); production always wires the real derivation.
...
impl RigReplyGenerator {
    ...
    /// snapshot; the live persona watcher rewrites it on each accepted
    /// reload when the persona name changes). The generator reads the
    /// current tag at every `generate` — the fence contract and the
    /// reply instruction follow the rename on the next call with no
    /// context/anchor change.
...
mod tests {
    ...
    /// The fence built from the legacy tag name `reply` — keeps the
    /// pre-95 fence fixtures byte-identical while the production tag
    /// comes from the persona name (decision 95).
    ...
    fn no_timezone_keeps_the_pre90_layout_byte_identical() {
        ...
        let instruction = render_reply_instruction(&request.target, "tamako");
        ...
        for mode in [SuffixMode::System, SuffixMode::Append] {
            let (_, messages) = assemble_reply_messages(&request, "tamako", "", mode, now, None);
            let rendered: Vec<(&str, String)> = messages.iter().map(role_and_text).collect();
        ...
        for (mode, expected) in [
            ...
            let (_, messages) =
                assemble_reply_messages(&request, "tamako", &suffix, mode, now, None);
            let rendered: Vec<(&str, String)> = messages.iter().map(role_and_text).collect();
    ...
    fn the_reply_instruction_names_the_target_verbatim() {
        let instruction = render_reply_instruction(&sample_target(), "tamako");
    ...
    fn the_reply_instruction_forbids_the_injection_format() {
        ...
        let instruction = render_reply_instruction(&sample_target(), "tamako");
        assert!(instruction
            .contains("Never write \"I remember:\" lines, <memory> blocks, <summary> blocks, <msg> blocks, or a memory list"));
    ...
    fn the_reply_instruction_states_the_f2_sentence_and_the_target_verbatim() {
        ...
        let expected = concat!(
            ...
            "Reply as the group pet persona. Write only the reply text: ",
            "one message, no speaker label, no quotes. ",
            "Wrap the whole reply in exactly one <tamako>...</tamako> element and write nothing outside it. ",
            "Never write \"I remember:\" lines, <memory> blocks, <summary> blocks, <msg> blocks, or a memory list: ",
            "recalled memories are context, never speech.",
        );
        assert_eq!(
            render_reply_instruction(&sample_target(), "tamako"),
            expected
        );
    }
